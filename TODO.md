...
- [ ] 既存Kioskのクーポン予約・確定・解除処理をorganization／LINE会員認証付きモバイル注文から利用可能にし、注文UUIDによる冪等性と同時利用防止を保証する
- [ ] モバイル注文の決済失敗・店舗却下・期限内キャンセル・Square返金時に、使用済みまたは予約中クーポンを安全に利用可能へ戻すAPIと監査履歴を追加する
- [ ] 予約注文でも予約期限切れによるクーポン解放が起きないよう、注文ライフサイクルに対応した予約・使用確定・復元タイミングを定義して結合試験する
